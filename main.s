...
QUIT		
			...
			BX	LR
;=========================================================			
READ_BUTTON_AND_WAIT

			MOV R0,	R2; копируем регистр флагов в аккумулятор
			BIC R0,R0,#0xFF; очищаем 1 и 2 тетраду
			CMP R0,#0X100; сравнение для проверки, какое состояние нужно получить при считывании
...
IF_STATE_1  AND R0,R0,#0x10           
			CMP R0 ,#WAITED_STATE_1	
			
			B	READ_QUIT
			
IF_STATE_0  AND R0,R0,#0x10           
			CMP R0 ,#WAITED_STATE_0	
			
			B	READ_QUIT
			
READ_QUIT	BNE READ_BUTTON_AND_WAIT	
			
			BX 	LR
;=========================================================			
STATE_SET	
			ADD BUTTON_STATE_COUNTER, #0x1
			MOV R0, BUTTON_STATE_COUNTER
			BIC	R0,#0xFF0
			CMP R0, #NUMBER_STATE_CONTROL
...
SYSDIV2		;reg RCC2->bit 28-23 SYSDIV2=0x02-> SELECT INPUT SYSTEM CLOCK DIVISOR = 5	
			LDR R1, =SYSCTL_RCC2_R
			LDR R0,	[R1]
			BIC R0, #0X0FF00000
			ORR R0, #0X02000000
...
GPIO_AHB_EN	;reg GPIOHBCTL->bit 5 R5=1-> ENABLE AHB BUS FOR GPIO PORT F	
			MOV R0, #0x20	
...
;TIMER0	properties - COUNTDOWN FROM HALF_SEC=0x1FCA055

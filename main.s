...
SSI0_EN		
			...
			STR R0, [R1]	
SSI0_GPIO_PORT_AF_ON ;reg RCGCGPIO->bit 1 R5=1-> ENABLE AND PROVIDE A CLOCK TO GPIO PORTS A, F IN RUN MODE	
			MOV R0, #0x21	
...
GPIO_AHB_EN	;reg GPIOHBCTL->bit 5 R5=1-> ENABLE AHB BUS FOR GPIO PORT F, A	
			MOV R0, #0x21	
			LDR R1, =SYSCTL_GPIOHBCTL_R 
			STR R0, [R1]
GPIO_A_DIR	;reg GPIODIR->bit  GPIODIR=0B1111.0100 -> SETUP PINS PF1,PF2,PF3 AS OUTPUTS
			MOV R0, #0xD0	
...
; GPIO properties - PB0,PB1,PB2 IS OUT FOR LCD
;PA6 - RST - RESET (RESET THEN 0)
;PA7 - CE - CHIP ENABLE (CHIP ENABLED THEN 0)
;PA4 - DC - DATA/COMMAND (DATA 1, COMMAND 0)
